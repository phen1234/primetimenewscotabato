export async function onRequest(context) { 
  ...
  try { 
    ...
    if (!firebaseResponse.ok) { 
      console.error( "Firestore request failed:", firebaseResponse.status ); 
      return context.next(); 
    } 
    ...
    const summary = getFirestoreValue( fields.summary ) || "Read the latest news from Primetime News Cotabato."; 
    
    let featuredImage = getFirestoreValue( fields.featuredImage ) || ""; 
    ...
    if (featuredImage) { 
      try { 
        ...
        console.error( "Invalid featured image URL:", featuredImage ); 
        featuredImage = ""; 
      } 
    } 
    ...
    if (!response) { 
      return new Response( "Article page could not be loaded.", { status: 500 } ); 
    } 
...
`;
    ...
    console.error( "Cloudflare Article Function Error:", error ); 
    
    try { 
      return await context.next(); 
    } catch (fallbackError) { 
      console.error( "Article fallback error:", fallbackError ); 
      return new Response( "Unable to load article.", { 
        status: 500, 
...
async function createFirebaseAccessToken( clientEmail, privateKey ) { 
  ...
  if (!tokenResponse.ok) { 
    ...
    console.error( "Firebase token error:", errorText ); 
