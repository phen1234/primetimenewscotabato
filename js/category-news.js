import { db } from "./firebase.js"; import { collection, getDocs, query, orderBy } from "https://www.gstatic.com/firebasejs/10.13.2/firebase-firestore.js";
// =============================== // PAGE INFO // ===============================
...
if (featuredContent) { featuredContent.innerHTML = `<div class="no-news"><h2> No ${currentPage.title} Found </h2><p> No published news available in this category. </p></div>`; }
if (contentList) { contentList.innerHTML = ""; }
if (sidebarContent) { sidebarContent.innerHTML = ""; }
return;
}
...
autoSlideNewsCards();
// PAANDARIN AUTO SLIDER PAG TAPOS NA LOAD
initAutoSlider();
} catch (err) {
console.error( "❌ CATEGORY NEWS ERROR:", err );
if (featuredContent) { featuredContent.innerHTML = `<div class="no-news"><h2> Failed to Load News </h2><p> Please check the browser console. </p></div>`; }
}
}
// =============================== // FEATURED NEWS // ===============================
...
featuredContent.innerHTML = `<div class="featured-news" data-id="${news.id}" ><img src="${image}" class="featured-image" alt="${headline}" onerror=" this.onerror=null; this.src='images/news1.jpg'; " ><div class="featured-overlay"><span class="badge"> ${category} </span><div class="news-meta"><span><i class="fas fa-calendar-alt"></i> ${publishedDate} </span><span><i class="fas fa-user"></i> ${author} </span><span><i class="fas fa-eye"></i> ${views} Views </span></div><h2> ${headline} </h2><p> ${summary} </p><button class="read-btn" data-id="${news.id}" type="button" > Read Full Story </button></div></div>`;
}
...
html += `<div class="news-card" data-id="${news.id}"><img src="${image}" alt="${headline}" loading="lazy" onerror="this.onerror=null; this.src='images/news1.jpg';"><div class="news-info"><span class="badge">${category}</span><h3>${headline}</h3><p>${summary}</p><div class="news-meta">${publishedDate? `<span><i class="fas fa-calendar-alt"></i> ${publishedDate}</span>` : ''}<span><i class="fas fa-eye"></i> ${views} Views</span></div></div></div>`;
});
...
const sortedByViews = [...newsList].sort((a, b) => { const viewsA = Number(a.views) || 0; const viewsB = Number(b.views) || 0; return viewsB - viewsA; });
...
const views = news.views || 0;
html += `<a href="article.html?id=${news.id}" class="most-read-item" data-id="${news.id}" ><img src="${image}" alt="${headline}" loading="lazy" onerror=" this.onerror=null; this.src='images/news1.jpg'; " ><div class="most-read-content"><span class="badge"> ${category} </span><h4> ${headline} </h4><small><i class="fas fa-eye"></i> ${views} Views </small></div></a>`;
});
...
if (featuredContent) { featuredContent.innerHTML = `<div class="no-news"><h2> No News Found </h2><p> No article matches your search. </p></div>`; }
if (contentList) { contentList.innerHTML = ""; }
if (sidebarContent) { sidebarContent.innerHTML = ""; }
}
});
...
menuToggle.addEventListener( "click", function (e) { e.stopPropagation(); navLinks.classList.toggle( "active" ); });
navLinks.querySelectorAll("a").forEach( function (link) { link.addEventListener( "click", function () { navLinks.classList.remove( "active" ); }); });
document.addEventListener( "click", function (e) { if (!navLinks.contains( e.target ) &&!menuToggle.contains( e.target ) ) { navLinks.classList.remove( "active" ); } });
});
...
// OPEN POPUP
if(filterBtn){ filterBtn.addEventListener('click', () => { filterPopup.classList.add('show'); filterOverlay.classList.add('show'); }); }
// CLOSE POPUP
function closePopup(){ filterPopup.classList.remove('show'); filterOverlay.classList.remove('show'); }
...
if(!selectedDate){ filteredNews = [...newsData]; }
else {
filteredNews = newsData.filter(news => {
const timestamp = news.publishedAt?.seconds || news.createdAt?.seconds;
if(timestamp){ const newsDate = new Date(timestamp * 1000); const newsDateString = newsDate.toISOString().split('T')[0]; return newsDateString === selectedDate; }
return false;
});
}
if(filteredNews.length > 0){ renderFeaturedNews(filteredNews[0]); renderNewsList(filteredNews.slice(1)); renderMostRead(filteredNews); }
else { featuredContent.innerHTML = `<div class="no-news"><h2>No News Found</h2><p>No article for this date.</p></div>`; contentList.innerHTML = ""; sidebarContent.innerHTML = ""; }
closePopup();
});
}
// CLEAR
...
// =============================== // AUTO SLIDER - CATEGORIES 1 BY 1 RIGHT TO LEFT ===============================
...
let i = 0;
function move(){ track.style.transform = 'translateX(' + (-i * 100) + '%)'; }
let autoSlide = setInterval(() => {
i++;
if(i >= cards.length) i = 0;
move();
}, 4000);
box.addEventListener('mouseenter', () => clearInterval(autoSlide));
